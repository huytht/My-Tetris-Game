...
public class Board : MonoBehaviour
{
    ...
    private bool IsWithinBoard(int x, int y) => (x >= 5.0f && x < width + 5 && y >= 0);

    private bool IsOccupied(int x, int y, Shape shape) =>
        grid[x - 5, y] != null && grid[x - 5, y].parent != shape.transform;
            ...
            for (int y = 0; y < height - header; y++)
            {
                for (float x = 0; x < width; x++)
                {
                    ...
                    clone = Instantiate<Transform>(
                        emptySprite, new Vector3(x + 5.0f, y, 0), Quaternion.identity);
                    clone.name =
    ...
    private void ClearRowFx(int idx, int y)
    {
        if (rowGlowFx[idx])
        {
            rowGlowFx[idx].transform.position = new Vector3(5, y, -2);
            rowGlowFx[idx].Play();
        }
    }
    ...
    public void StoreShapeInGrid(Shape shape)
    {
        ...
        {
            Vector2 pos = Vectorf.Round(child.position);
            grid[(int)pos.x - 5, (int)pos.y] = child;
        }
    }
